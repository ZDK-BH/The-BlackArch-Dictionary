...
**Category:** `automation`  
**Version in source list:** `82.4684670`  
**Package name:** `awsbucketdump`
...
```bash
sudo pacman -S --needed awsbucketdump
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql awsbucketdump | grep '/bin/'
```
...
awsbucketdump -i targets.txt -o results.txt
```
