...
**Package name:** `ssh-privkey-crack`
...
```bash
sudo pacman -S --needed ssh-privkey-crack
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql ssh-privkey-crack | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
ssh-privkey-crack --help
ssh-privkey-crack -h
man ssh-privkey-crack
```
...
ssh-privkey-crack ./sample_hashes.txt ./wordlist.txt
```
