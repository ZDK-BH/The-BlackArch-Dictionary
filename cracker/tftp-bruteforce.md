...
```bash
sudo pacman -S --needed tftp-bruteforce
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql tftp-bruteforce | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
tftp-bruteforce --help
tftp-bruteforce -h
man tftp-bruteforce
```
...
tftp-bruteforce ./sample_hashes.txt ./wordlist.txt
```
