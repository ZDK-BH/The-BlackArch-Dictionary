...
**Version in source list:** `83.f1e596f`  
**Package name:** `neighbor-cache-fingerprinter`
...
```bash
sudo pacman -S --needed neighbor-cache-fingerprinter
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql neighbor-cache-fingerprinter | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
neighbor-cache-fingerprinter --help
neighbor-cache-fingerprinter -h
man neighbor-cache-fingerprinter
```
...
neighbor-cache-fingerprinter https://example.test
```
