...
```bash
sudo pacman -S --needed ntp-fingerprint
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql ntp-fingerprint | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
ntp-fingerprint --help
ntp-fingerprint -h
man ntp-fingerprint
```
...
ntp-fingerprint https://example.test
```
