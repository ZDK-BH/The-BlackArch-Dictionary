...
**Package name:** `heartbleed-honeypot`
...
```bash
sudo pacman -S --needed heartbleed-honeypot
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql heartbleed-honeypot | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
heartbleed-honeypot --help
heartbleed-honeypot -h
man heartbleed-honeypot
```
