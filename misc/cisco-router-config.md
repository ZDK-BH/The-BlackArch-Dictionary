...
**Package name:** `cisco-router-config`
...
```bash
sudo pacman -S --needed cisco-router-config
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql cisco-router-config | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
cisco-router-config --help
cisco-router-config -h
man cisco-router-config
```
...
```bash
pacman -Ql cisco-router-config | sed -n "1,120p"
```
