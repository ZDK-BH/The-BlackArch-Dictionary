...
**Category:** `misc`  
**Version in source list:** `23.1e5ceb1`  
**Package name:** `dumpsmbshare`
...
```bash
sudo pacman -S --needed dumpsmbshare
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql dumpsmbshare | grep '/bin/'
```
...
```bash
pacman -Ql dumpsmbshare | sed -n "1,120p"
```
