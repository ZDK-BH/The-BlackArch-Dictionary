...
**Category:** `misc`  
**Version in source list:** `26.a8e31b3`  
**Package name:** `nmap-parse-output`
...
```bash
sudo pacman -S --needed nmap-parse-output
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql nmap-parse-output | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
nmap-parse-output --help
nmap-parse-output -h
man nmap-parse-output
```
...
```bash
pacman -Ql nmap-parse-output | sed -n "1,120p"
```
