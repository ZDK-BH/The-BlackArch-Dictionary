...
```bash
sudo pacman -S --needed python2-utidylib
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql python2-utidylib | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
python2-utidylib --help
python2-utidylib -h
man python2-utidylib
```
...
```bash
pacman -Ql python2-utidylib | sed -n "1,120p"
```
