...
**Category:** `misc`  
**Version in source list:** `2.78ebbfc`  
**Package name:** `rogue-mysql-server`
...
```bash
sudo pacman -S --needed rogue-mysql-server
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql rogue-mysql-server | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
rogue-mysql-server --help
rogue-mysql-server -h
man rogue-mysql-server
```
...
```bash
pacman -Ql rogue-mysql-server | sed -n "1,120p"
```
