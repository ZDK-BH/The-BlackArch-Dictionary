...
**Category:** `misc`  
**Version in source list:** `suricata.8.0.5.r63.g26caf82`  
**Package name:** `suricata-verify`
...
```bash
sudo pacman -S --needed suricata-verify
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql suricata-verify | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
suricata-verify --help
suricata-verify -h
man suricata-verify
```
...
```bash
pacman -Ql suricata-verify | sed -n "1,120p"
```
