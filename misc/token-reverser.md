...
**Category:** `misc`  
**Version in source list:** `10.cbb4528`  
**Package name:** `token-reverser`
...
```bash
sudo pacman -S --needed token-reverser
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql token-reverser | grep '/bin/'
```
...
```bash
pacman -Ql token-reverser | sed -n "1,120p"
```
