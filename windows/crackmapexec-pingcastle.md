...
**Package name:** `crackmapexec-pingcastle`
...
```bash
sudo pacman -S --needed crackmapexec-pingcastle
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql crackmapexec-pingcastle | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
crackmapexec-pingcastle --help
crackmapexec-pingcastle -h
man crackmapexec-pingcastle
```
...
```bash
# Package contents:
pacman -Ql crackmapexec-pingcastle | sed -n "1,80p"
```
