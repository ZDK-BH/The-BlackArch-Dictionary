...
```bash
sudo pacman -S --needed sqlpowerinjector
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql sqlpowerinjector | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
sqlpowerinjector --help
sqlpowerinjector -h
man sqlpowerinjector
```
...
```bash
# Package contents:
pacman -Ql sqlpowerinjector | sed -n "1,80p"
```
