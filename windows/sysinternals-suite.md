...
**Category:** `windows`  
**Version in source list:** `6.4`  
**Package name:** `sysinternals-suite`
...
```bash
sudo pacman -S --needed sysinternals-suite
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql sysinternals-suite | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
sysinternals-suite --help
sysinternals-suite -h
man sysinternals-suite
```
...
```bash
# Package contents:
pacman -Ql sysinternals-suite | sed -n "1,80p"
```
