...
**Category:** `windows`  
**Version in source list:** `27.ebed079`  
**Package name:** `targetedkerberoast`
...
```bash
sudo pacman -S --needed targetedkerberoast
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql targetedkerberoast | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
targetedkerberoast --help
targetedkerberoast -h
man targetedkerberoast
```
...
```bash
# Package contents:
pacman -Ql targetedkerberoast | sed -n "1,80p"
```
