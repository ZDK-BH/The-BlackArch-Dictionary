...
**Category:** `windows`  
**Version in source list:** `20.7d272da`  
...
```bash
sudo pacman -S --needed windows-binaries
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql windows-binaries | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
windows-binaries --help
windows-binaries -h
man windows-binaries
```
...
```bash
# Package contents:
pacman -Ql windows-binaries | sed -n "1,80p"
```
