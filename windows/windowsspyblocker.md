...
**Category:** `windows`  
**Version in source list:** `4.38.0`  
**Package name:** `windowsspyblocker`
...
```bash
sudo pacman -S --needed windowsspyblocker
```

Confirm the installed files and find the actual executable name:

```bash
pacman -Ql windowsspyblocker | grep '/bin/'
```

Open the built-in help before running anything meaningful:

```bash
windowsspyblocker --help
windowsspyblocker -h
man windowsspyblocker
```
...
```bash
# Package contents:
pacman -Ql windowsspyblocker | sed -n "1,80p"
```
